...
bool CustomBLEScanner::parse_device(const esp32_ble_tracker::ESPBTDevice &device) {
  // Corrected: Get raw advertisement data directly from the device object
  ...
  // Use get_service_datas() to check for service UUIDs
...
bool CustomBLEScanner::parse_bthome_v2_device(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  optional<std::string> received_advertised_name_opt = device.get_name(); // get_name returns optional<std::string>
  ...
  while (offset < bthome_data.size()) {
    ...
    switch (type) {
      ...
      case BTHOME_MEASUREMENT_TEMPERATURE: {
        ...
        // You generally shouldn't publish directly to individual ESPHome sensors from here
        // if you're doing MQTT discovery. The MQTT discovery will handle HA sensors.
        // However, if these are *also* ESPHome native sensors, this is fine.
